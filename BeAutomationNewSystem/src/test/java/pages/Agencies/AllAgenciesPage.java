...
//        searchBarAllRoles(driver);
    ...
    public void openUsersPage(WebDriver driver) throws IOException, AWTException {
        AllAgenciesUsersPage newUser=new AllAgenciesUsersPage(driver);
        WebDriverWait wait = new WebDriverWait(driver,Duration.ofSeconds(7));
        newRobot.delay(1000);
        openActionsMenu();
        agencyUsersButton.click();
        wait.until(ExpectedConditions.visibilityOf(newUser.newAgencyUserButton));
        newUser.newAgencyUserButton.click();
        newUser.clickOnNewAgencyUserButton(driver);
    }
    public void openAgencyCreditsPage(WebDriver driver) throws AWTException {
        AllAgenciesCreditPage creditsPage=new AllAgenciesCreditPage(driver);
        WebDriverWait wait = new WebDriverWait(driver,Duration.ofSeconds(7));
        newRobot.delay(1000);
        openActionsMenu();
        agencyCreditButton.click();
        wait.until(ExpectedConditions.urlMatches(creditsPage.creditsPageURL));
        creditsPage.newAgencyCredit(driver);
    }
